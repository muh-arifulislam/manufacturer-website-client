...
const MyProfile = () => {
  ...
  const handleSubmit = () => {
    ...
    axios
      .put(
        "https://manufacturer-website-server-production-7476.up.railway.app/user",
        data,
        {
          headers: {
            authorization: `Bearer ${localStorage.getItem("accessToken")}`,
    ...
    fetch(url, {
      ...
      .then((result) => {
        ...
        if (result.success) {
          ...
          fetch(
            `https://manufacturer-website-server-production-7476.up.railway.app/user/image/${userData._id}?email=${user.email}`,
            {
              method: "PUT",
              headers: {
  ...
  return (
    <section className=" p-4">
      ...
      <div className="flex lg:flex-row flex-col items-center justify-center gap-[15px]">
        ...
        <div>
          <form
            onSubmit={handleImageSubmit(onSubmit)}
          ...
          >
            ...
            <input
              type="submit"
              className="btn btn-primary btn-sm px-[15px] normal-case mt-[12px] text-white"
          ...
          <div className="lg:flex items-end gap-[30px]  mt-[12px]">
            ...
            <div className="form-control w-full lg:mt-[0] mt-[12px]">
              <div className="flex items-center">
                <input
                  onChange={(e) => setEducationFromYear(e.target.value)}
                  type="text"
                  placeholder="from"
                  className="input input-bordered w-full max-w-auto"
                  value={educationFromYear}
                />
                <p className="text-lg mx-[10px] font-bold">year</p>
                <input
                  onChange={(e) => setEducationToYear(e.target.value)}
                  type="text"
                  placeholder="to"
                  className="input input-bordered  w-full max-w-auto"
                  value={educationToYear}
                />
              </div>
            </div>
          </div>
